function curved_lines_gradient_2() {
  ...
  const wrapper = d3.select("#curve-lines-gradient-2")
  ...
  data.forEach(el => {
    ...
    bounds.append("path")
      ...
      .style("stroke", function(d,i) {
        ...
        const gradientID = `gradient${el}` // make unique gradient ids  
  ...
  function animatePaths() {
    ...
    bounds.selectAll(".line").each(function(d,i){
      repeat()
      function repeat() {
        var totalLength = bounds.select("#line" + i).node().getTotalLength() // Get the length of each line in turn
        bounds.selectAll("#line" + i).attr("stroke-dasharray", totalLength + " " + totalLength) 
